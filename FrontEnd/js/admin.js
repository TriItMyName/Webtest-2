...
btnAddProductIn.addEventListener("click", (e) => {
    ...
    if(tenMon == "" || price == "" || moTa == "") {
        toast({ title: "Chú ý", message: "Vui lòng nhập đầy đủ thông tin môn học!", type: "warning", duration: 3000, });
    } else {
        if(isNaN(price)) {
...
function setDefaultValue() {
    ...
    document.getElementById("chon-mon").value = "Chọn môn học";
}
...
// Show số lượng sp, số lượng đơn bán, số lượng học viên
function showOverview(arr){
    ...
    document.getElementById("quantity-user").innerText = arr.reduce((sum, cur) => (sum + parseInt(cur.soluong)),0);
}
...
function showUserArr(arr) {
    let accountHtml = '';
    if(arr.length == 0) {
        accountHtml = `<td colspan="4">Không có dữ liệu</td>`; // Adjust colspan
    } else {
        arr.forEach((account, index) => {
            ...
            <button class="btn-edit" id="edit-account" onclick='editAccount(${index})' ><i class="fa-solid fa-pen-to-square"></i></button>
...
addAccount.addEventListener("click", (e) => {
    ...
    if (fullNameUser && passwordUser) {
        ...
        let accounts = localStorage.getItem('accounts') ? JSON.parse(localStorage.getItem('accounts')) : [];
        let checkloop = accounts.some(account => account.fullname == user.fullname);
        if (!checkloop) {
            accounts.push(user);
            localStorage.setItem('accounts', JSON.stringify(accounts));
